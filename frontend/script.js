 ...
 * 1) window.__API_BASE_URL__ from api-config.js (.env PUBLIC_API_URL or Vercel build)
 * 2) Same origin when page is served over http(s) — tek Node sunucusunda
 * 3) file:// veya yapılandırma yoksa: 127.0.0.1 + __DEV_API_PORT__
 */
function getApiCandidates() {
  ...
  if (/^https?:\/\//i.test(hostOrigin)) {
    return [normalizeBaseUrl(hostOrigin)];
  }

  const port = String(window.__DEV_API_PORT__ || "3000").trim() || "3000";
  // file:// ile açıldığında bazı ortamlarda 127.0.0.1 / localhost farkı veya DNS nedeniyle biri çalışır.
...
async function callApi(endpoint, outputEl, loadingText) {
  ...
  if (endpoint === "analyze" && !Number.isFinite(payload.measured)) {
    setOutput(outputEl, "Analiz için Ölçülen Değer alanına geçerli sayı giriniz.");
    return null;
  }
  if (endpoint === "analyze" && payload.temperature !== 0 && !Number.isFinite(payload.temperature)) {
    payload.temperature = 25;
  }
  if (endpoint !== "analyze" && !Number.isFinite(payload.temperature)) {
    payload.temperature = 25;
  }
  ...
  try {
    ...
    const hint = isFileProtocol()
      ? `Sayfa dosyadan (file://) açıldığı için API çağrısı çoğu tarayıcıda engellenebilir veya sunucuya ulaşılamıyor. Çözüm: proje klasöründe "npm start" çalıştırın ve adres çubuğuna ${localUrl} yazın (çift tıklama yerine).`
      : `Sunucuya ulaşılamıyor. Yerelde "npm start" ile backend’i başlatıp ${localUrl} üzerinden açmayı deneyin. Uzak ortamda PUBLIC_API_URL / API adresini kontrol edin.`;
...
analyzeBtn.addEventListener("click", async () => {
  const data = await callApi("analyze", analysisOutput, "AI düşünüyor...");
  if (!data) return;
...
teachBtn.addEventListener("click", async () => {
  const data = await callApi("teach", teachingOutput, "AI düşünüyor...");
  if (!data) return;
...
simulateBtn.addEventListener("click", async () => {
  const data = await callApi("simulate", simulationOutput, "AI düşünüyor...");
  if (!data) return;
  console.log(data.result);
  setOutput(
    simulationOutput,
